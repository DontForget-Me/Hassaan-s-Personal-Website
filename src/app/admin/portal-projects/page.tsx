...
export default function AdminPortalProjectsPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch('/api/admin/portal-projects' + (filter ? `?status=${filter}` : ''));
        if (res.ok) {
    ...
    <div className="min-h-screen" style={{ backgroundColor: 'var(--bg-primary)' }}>
      ...
      <main className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h1 className="text-xl font-light sm:text-2xl" style={{ color: 'var(--text-primary)' }}>Client Projects</h1>
            <p className="mt-1 text-sm" style={{ color: 'var(--text-secondary)' }}>Manage all client portal projects.</p>
        ...
        <div className="space-y-3">
          {projects.map((project) => {
            ...
            return (
              <Link key={project.id} href={`/admin/portal-projects/${project.id}`}
                className="flex items-center justify-between rounded-2xl border p-5 transition-all"
                style={{ backgroundColor: 'var(--surface)', borderColor: 'var(--border)' }}
                onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'var(--accent)'; }}
                onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'var(--border)'; }}>
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-medium" style={{ color: 'var(--text-primary)' }}>{project.title}</p>
